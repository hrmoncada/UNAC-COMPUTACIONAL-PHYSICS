...
//http://fracton.khu.ac.kr/~syook/Lectures/ComputPhys/C/NumCalculus/BuffonNeedle.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
int throw_needles()
{
	...
	if(gchkflag){
		...
		for(i=0,ncross=0;i<gnneedles;i++){
		...
		glBegin(GL_POINTS);
		for(i=0;i<gi;i++){
			px=i*DN*(B2WIDTH-ADX)/20000.0+RDX+B1WIDTH+BBDX+ADX;
			py=gpi[i]*(B2HEIGHT-ADY)/4.0+LDY+ADY;
			glVertex2f(px,py);
		}
		glEnd();
		//For text information.
		glColor3f(1.0,1.0,0.0);
		glRasterPos2f(LDX,LDY-20);
		sprintf(label,"A=[Tot. Num. of Needles]=%d\n",gnneedles);
		drawString(GLUT_BITMAP_HELVETICA_12,label);
		glRasterPos2f(LDX,LDY-33);
		sprintf(label,"B=[Num. of Needles Crossed the Bars]=%d\n",ncross);
		drawString(GLUT_BITMAP_HELVETICA_12,label);
		glRasterPos2f(LDX,LDY-46);
		sprintf(label,"Pi=A/B=%.10lf\n",pi);
		drawString(GLUT_BITMAP_HELVETICA_12,label);
